...
#### 根目录
- config.py：定义各种客户端类参数配置对象，其中get_settings()方法，返回Settings，里面定义了各种基础参数及客户端类参数配置对象。
- 
#### services模块
- arxiv模块：定义arxiv论文爬虫客户端`ArxivClient`类，该类定义了爬取网页、解析网页、下载网页方法。爬取网页过程中添加了频率限制及错误重试机制。通过解析网页方法，提取网页内容，返回`ArxivPaper`对象列表或者单个对象。下载网页的方法可以将网页保存成PDF格式文档。
  ```python
